...
def _chunk_data(data_bytes: bytes) -> list[bytes]:
    ...
    return [
        data_bytes[i : i + config.CHUNK_SIZE]
        for i in range(0, len(data_bytes), config.CHUNK_SIZE)
    ]
...
def _check_if_told_to_stop(
    ...
    if sock in ready_to_read:
        signal, received_counter = utility.recv_signal_tcp(
            sock
        )  # Read just 1 byte non-blockingly (due to select)
        if received_counter != expected_counter:
            print(
                f"MLT: Warning - Received counter {received_counter} does not match expected counter {expected_counter}."
            )
            return False, None
        if not signal:  # Connection closed
            raise ConnectionError(
                "MLT HELPER: Connection closed by receiver during early check."
            )
        if signal == b"S":
            print(
                "MLT HELPER: Received early 'Stop' (S) from server. Transmission for this gradient complete."
            )
            return True, None
        elif signal == b"B":
            ...
            if not new_bitmap_data or len(new_bitmap_data) != bitmap_len_to_recv:
                raise ConnectionError(
                    "MLT HELPER ERROR: Failed to receive complete bitmap data from server."
                )

            if (
                bytearray(new_bitmap_data) == server_ack_bitmap
                and len(new_bitmap_data) > 0
            ):
...
def serialize_gradient_to_custom_binary(
    tcp_sock: socket.socket, key: str, tensor: torch.Tensor
):
    """
...
def send_data_mlt(
    ...
    try:
        ...
        # Send metadata first
        utility.send_signal_tcp(tcp_sock, b"M", signal_counter)
        utility.send_data_tcp(tcp_sock, num_chunks)

        if config.DEBUG:
            print(f"SENDER MLT: NUM_CHUNKS sent with {num_chunks} chunks.")
        ...
        while True:
            ...
            if bitmap_same_from_last_round:
                if config.DEBUG:
                    print(
                        "SENDER MLT: Skipping chunk sending loop due to same bitmap from last round."
                    )
            else:
                for i in range(num_chunks):
                    ...
                    if not ((server_ack_bitmap[byte_idx] >> bit_idx) & 1):
                        ...
                        if config.DEBUG:
                            ...
                            time_with_ms = (
                                f"{now:%Y-%m-%d %H:%M:%S}.{now.microsecond // 1000:03d}"
                            )
                            print(
                                f"SENDER MLT: Sent chunk {i}/{num_chunks} via UDP at {time_with_ms}."
                            )
                    
                    # ZM on 8.3.2025: I think even once of this signal checking over 2000+ chunks just for 1 communication is too much
                    # As a result, I want to see if checking it every 50 chunks is enough
                    # ZM on 8.7.2025: reduced from 100 to 50 as this is a per-val communication
                    if i % 50 == 0:  # Check every 50 chunks
                        # Check if the sender has sent a 'Stop' (S) or 'Bitmap' (B) signal
                        if config.DEBUG:
                            print(
                                f"SENDER MLT: Checking for 'Stop' signal at chunk {i}/{num_chunks}."
                            )
                        cond, new_bitmap = _check_if_told_to_stop(
                            tcp_sock, signal_counter, server_ack_bitmap
                        )
                        if cond:
                            if config.DEBUG:
                                print(
                                    "SENDER MLT: 'Stop' signal received at the beginning of for loop. Transmission COMPLETED."
                                )
                                print(
                                    f"SENDER MLT: Sent {chunks_sent_this_round} UDP chunks this round"
                                )
                            return True
                        else:
                            if (
                                new_bitmap is not None
                                and new_bitmap != server_ack_bitmap
                            ):
            ...
            while True:
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"SENDER MLT ERROR: Failed to send 'Probe' (P) signal: {e}"
                    )
                ...
                while not ready_to_read:
                    ...
                    if probe_cnt > 10:  # Arbitrary limit to avoid infinite loop
                        print(
                            f"SENDER MLT: No response from server after 'Probe' (P) after {probe_cnt} attempts. Will re-Probe."
                        )
                        break
                    ...
                    if config.DEBUG:
                        ...
                        print(
                            f"SENDER MLT: No response from server after 'Probe' (P) at {time_with_ms} ({probe_cnt} / 10). Retrying..."
                        )
            ...
            if not signal:
                raise ConnectionError(
                    "SENDER MLT ERROR: Connection closed by receiver while waiting for response."
                )

            # CRITICAL CHANGE: Check if the received counter matches the expected signal_counter
            while received_counter != signal_counter:
                print(
                    f"SENDER MLT: Received {signal} signal with counter {received_counter}. Expected: {signal_counter}. Retrying..."
                )
                signal, received_counter = utility.recv_signal_tcp(tcp_sock)
            ...
            if signal == b"S":
                ...
                if not new_bitmap_data or len(new_bitmap_data) != bitmap_len_to_recv:
                    raise ConnectionError(
                        "SENDER MLT ERROR: Failed to receive complete bitmap data from server."
                    )
                ...
                if bytearray(new_bitmap_data) == server_ack_bitmap:
                    ...
                    print(
                        f"SENDER MLT: No change in bitmap. Progress stalled ({no_progress_rounds}/{max_retries_no_progress})."
                    )

                    if no_progress_rounds > max_retries_no_progress:
                        # to prevent deadlock
                        print(
                            "SENDER MLT: Max retries with no progress reached. Will retransmit UDP chunks."
                        )
                        bitmap_same_from_last_round = False
                        no_progress_rounds = 0
                else:
                ...
                print(
                    f"SENDER MLT ERROR: Unrecognized signal '{signal}' from receiver."
                )
                return False
...
def recv_data_mlt(
    socks: dict, tcp_addr: tuple, expected_counter: int, metadata_list
) -> tuple[dict | None, tuple | None, int] | None:
    """
    ...
    if not eval_signal:
        if config.DEBUG:
            print(
                "RECEIVER: Did not receive initial eval signal. Connection may be closed."
            )
        return None
    ...
    if eval_signal == b"E":
        ...
        if not eval_acc_bytes or not epoch_bytes:
            raise ValueError(
                "RECEIVER ERROR: Failed to receive eval data after 'E' signal."
            )
        ...
        print(
            f"[Worker {tcp_addr}] RECEIVER: Received 'E' signal with eval_acc={eval_acc}, epoch={curr_epoch}"
        )
    # Case 2: signal is 'N' (no eval data will be sent)
        ...
        print(
            f"[Worker {tcp_addr}] RECEIVER ERROR: Unrecognized initial signal '{eval_signal}'."
        )
        return None
    ...
    for i in range(len(metadata_list)):
        ...
        while signal != b"M" or received_counter != expected_counter:
            print(
                f"[Worker {tcp_addr}] RECEIVER MLT: Received {signal} signal with counter {received_counter}. Expected: M with counter {expected_counter}. Retrying... ({retry_count})"
            )
            
            if retry_count > 20:
                raise ValueError(
                    f"[Worker {tcp_addr}] RECEIVER MLT ERROR: Too many retries ({retry_count}) for signal. Exiting."
                )
        ...
        if config.DEBUG:
            print(
                f"[Worker {tcp_addr}] RECEIVER MLT: Received 'M' signal with counter {received_counter} from sender."
            )
        ...
        if config.DEBUG:
            print(
                f"[Worker {tcp_addr}] RECEIVER MLT: Preparing to receive gradient {i}/{len(metadata_list)}: {metadata_list[i]}"
            )    
        ...
        while None in received_chunks:
            try:
                ...
                if udp_sock in readable:
                    ...
                    if len(packet) < 12:
                        print(
                            f"[Worker {tcp_addr}] RECEIVER MLT(UDP): Packet too small: {len(packet)} bytes. Ignoring."
                        )
                        continue
                    ...
                    if received_signal_counter != expected_counter:
                        print(
                            f"[Worker {tcp_addr}] RECEIVER MLT(UDP): Received counter {received_signal_counter} does not match expected counter {expected_counter}. Ignoring packet."
                        )
                        continue
                    ...
                    if (
                        ...
                        if config.DEBUG:
                            print(
                                f"[Worker {tcp_addr}] RECEIVER MLT(UDP): Received chunk {seq}/{num_chunks} ({chunk_len_in_header} bytes) from {udp_addr} at {time_with_ms}."
                            )
                    
                    # Check for early stop signal
                    if (
                        num_chunks > 0
                        and (received_chunks.count(None) / num_chunks)
                        <= config.loss_tolerance
                    ):
                        if config.DEBUG:
                            print(
                                f"[Worker {tcp_addr}] RECEIVER MLT: Loss tolerance ({config.loss_tolerance}) met. Sending 'Stop' (S)."
                            )
                        has_stopped = True
                ...
                if tcp_sock in readable:
                    if config.DEBUG:
                        print(
                            f"[Worker {tcp_addr}] RECEIVER MLT(TCP): TCP socket is readable. Waiting for signal from sender at {time_with_ms}..."
                        )
                    signal, received_counter = utility.recv_signal_tcp(tcp_sock)

                    if not signal:
                        raise ValueError(
                            f"[Worker {tcp_addr}] RECEIVER ERROR: Failed to receive signal from TCP socket."
                        )
                    ...
                    if signal == b"P":
                        ...
                        if (
                            received_chunks.count(None) / num_chunks
                        ) <= config.loss_tolerance:
                            if config.DEBUG:
                                print(
                                    f"[Worker {tcp_addr}] RECEIVER MLT: Loss tolerance ({config.loss_tolerance}) met. Sending 'Stop' (S)."
                            ...
                            if config.DEBUG:
                                print(
                                    f"[Worker {tcp_addr}] RECEIVER MLT: Sending 'Bitmap' (B) {bitmap} with counter {received_counter} to sender.  "
                                )
                        ...
                        raise ConnectionError(
                            "RECEIVER MLT: Did not receive proper Probe (P) signal from sender."
                        )
        ...
        if not has_stopped:
            if config.DEBUG:
                print(
                    "RECEIVER MLT: got out of chunk send/recv loop but had not sent STOP signal yet."
                )
            utility.send_signal_tcp(tcp_sock, b"S", expected_counter)

        # ------------- Final Phase: Reconstruct Gradients -------------
        for j, chunk in enumerate(received_chunks):
            if chunk is None:
                ...
                received_chunks[j] = (
                    b"\x00" * config.CHUNK_SIZE
                )  # Fill missing chunks with empty bytes
        ...
        if not isinstance(metadata, dict):
            raise ValueError(
                f"[Worker {tcp_addr}] RECEIVER ERROR: Metadata must be a dictionary. Got {type(metadata)}."
            )
        ...
        if shape is None:
            raise ValueError(
                f"[Worker {tcp_addr}] RECEIVER ERROR: Tensor shape is None for key '{key}'."
            )
        if tensor_data_len_expected is None:
            raise ValueError(
                f"[Worker {tcp_addr}] RECEIVER ERROR: Expected tensor data length is None for key '{key}'."
            )
        if key is None:
            raise ValueError(
                f"[Worker {tcp_addr}] RECEIVER ERROR: Key is None in metadata for tensor reconstruction."
            )

        reconstructed_tensor = torch.zeros(shape, dtype=torch_dtype)

        if tensor_data_len_expected > 0:
            if len(concatenated_data) < tensor_data_len_expected:
                raise ValueError(
                    f"[Worker {tcp_addr}] RECEIVER ERROR: Concatenated data length {len(concatenated_data)} is less than expected {tensor_data_len_expected} for key '{key}'."
                )
            if len(concatenated_data) > tensor_data_len_expected:
                print(
                    f"[Worker {tcp_addr}] RECEIVER WARNING: Concatenated data length {len(concatenated_data)} is greater than expected {tensor_data_len_expected} for key '{key}'. Truncating."
                )
            # Truncate to the expected length
            tensor_data = concatenated_data[:tensor_data_len_expected]
    ...
    return final_gradients_dict, udp_addr, expected_counter
