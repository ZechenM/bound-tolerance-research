...
class DistributedTrainerMultithreadingPerVal(Trainer):
    ...
    def print_total_network_latency(self):
        ...
        if self.network_latency_list:
            ...
            print(
                f"[Worker {self.id}] Total network latency: {total_latency:.6f} seconds"
            )
        else:
    ...
    def training_step(self, model, inputs, num_items_in_batch):
        ...
        print(
            f"Training Step Running - Worker {self.id}, Current Epoch: {current_epoch}"
        )
        ...
        if current_epoch is not None:
            ...
            if (
                abs(int(current_epoch) - current_epoch) < 1e-10
                and epoch_diff > 0.999
                or current_epoch == 0
            ):
                ...
                self.sent_eval = True
                print(
                    f"Current epoch: {current_epoch}, Self curr epoch: {self.curr_epoch}"
                )
        ...
        # instead, we send a signal to tell the receiver whether we are sending "epoch" and "eval_acc" or not
        ...
        if not self.has_sent_metadata:
            for key, tensor in gradients.items():
                if not isinstance(tensor, torch.Tensor):
                    raise ValueError(
                        f"[Worker {self.id}] Expected tensor, got {type(tensor)} for key '{key}'"
                    )
        ...
        result = mlt_per_val.recv_data_mlt(
            socks, (self.server_host, self.tcp_port), self.signal_counter, self.metadata_list
        )
        ...
        averaged_gradients, _, counter = result
        old_signal_counter = self.signal_counter
        self.signal_counter = counter
        if averaged_gradients is None:
            raise ValueError(
                f"[WORKER {self.id}] failed to receive averaged gradients back from the server"
            )
        
        if config.DEBUG:
            print(
                f"[Worker {self.id}] Received averaged gradients: {len(averaged_gradients)} items."
            )
            for key, tensor in averaged_gradients.items():
                print(f"  - {key}: {tensor.shape} on device {tensor.device}")

            print(f"[Worker {self.id}] Old signal counter after receiving gradients: {old_signal_counter}")
            print(f"[Worker {self.id}] New signal counter after receiving gradients: {self.signal_counter}")
    ...
    def send_gradients(self, gradients_dict):
        ...
        if self.sent_eval:
            ...
            print(
                f"WORKER: Sent eval signal 'E' with data: acc={self.eval_acc}, epoch={self.curr_epoch}"
            )
        else:
        ...
        for key, tensor in gradients_dict.items():
            ...
            if payload_bytes is None:
                raise ValueError(
                    f"[Worker {self.id}] Failed to serialize tensor data for key '{key}'."
                )

            success = mlt_per_val.send_data_mlt(
                socks, addrs, payload_bytes, self.signal_counter
            )
            self.signal_counter += 1

            if not success:
                raise ValueError(
                    f"[Worker {self.id}] Failed to send tensor data using MLT protocol."
                )
    ...
    def train(self, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None):
        ...
        resume_checkpoint = (
            ...
            else self.args.resume_from_checkpoint
        )
            ...
            if isinstance(resume_checkpoint, str) and "checkpoint" in resume_checkpoint:
                try:
                    ...
                    if self.args.max_steps <= checkpoint_step:
                        print(
                            f"Adjusting max_steps from {self.args.max_steps} to {checkpoint_step + 20500}"
                        )
                        self.args.max_steps = (
                            checkpoint_step + 20500
                        )  # should be double if I want to do 2 time more epoch
                except Exception:
