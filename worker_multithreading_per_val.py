...
import config
from distributed_trainer_multithreading_per_val import DistributedTrainerMultithreadingPerVal
...
class Worker:
    def __init__(self, worker_id, server_host, tcp_port=9999):
        ...
        print(
            f"Worker {self.worker_id} WILL BE connecting to server at {self.server_host}:{self.server_port}"
        )
    ...
    def find_latest_checkpoint(self):
        ...
        latest_checkpoint = os.path.join(
            checkpoint_dir, f"checkpoint-{latest_checkpoint_num}"
        )
    ...
    def train_worker(self):
        ...
        if latest_checkpoint:
            print(
                f"Found latest checkpoint at {latest_checkpoint}. Will resume training from this point."
            )
        else:
            print("No checkpoint found. Starting training from scratch.")

        trainer = DistributedTrainerMultithreadingPerVal(
            model=self.model,
